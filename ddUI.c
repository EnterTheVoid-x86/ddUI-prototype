 ...
 *
 * The event handlers of ddUI are organized in an array which is accessed
...
buttonpress(XEvent *e)
{
	...
	#if BAR_STATUSCMD_PATCH && !BAR_ddUIBLOCKS_PATCH
	*lastbutton = '0' + ev->button;
	#endif // BAR_STATUSCMD_PATCH | BAR_ddUIBLOCKS_PATCH
...
clientmessage(XEvent *e)
{
	...
	if (showsystray && systray && cme->window == systray->win && cme->message_type == netatom[NetSystemTrayOP]) {
		...
		if (cme->data.l[1] == SYSTEM_TRAY_REQUEST_DOCK) {
			...
			XClassHint ch = {"ddUIsystray", "ddUIsystray"};
...
propertynotify(XEvent *e)
{
	...
	if ((ev->window == root) && (ev->atom == XA_WM_NAME)) {
		#if ddUIC_PATCH || FSIGNAL_PATCH
		if (!fake_signal())
			updatestatus();
		#else
		updatestatus();
		#endif // ddUIC_PATCH / FSIGNAL_PATCH
...
quit(const Arg *arg)
{
	...
	if (n <= quit_empty_window_count)
		...
		fprintf(stderr, "[ddUI] not exiting (n=%d)\n", n);
...
setup(void)
{
	...
	#endif // WINDOWROLERULE_PATCH
	#if SEAMLESS_RESTART_PATCH
	clientatom[ClientFields] = XInternAtom(dpy, "_ddUI_CLIENT_FIELDS", False);
	clientatom[ClientTags] = XInternAtom(dpy, "_ddUI_CLIENT_TAGS", False);
	#endif // SEAMLESS_RESTART_PATCH
	...
	#else
	XChangeProperty(dpy, wmcheckwin, netatom[NetWMName], utf8string, 8,
		PropModeReplace, (unsigned char *) "ddUI", 3);
	#endif // LG3D_PATCH
...
#endif // RIODRAW_PATCH
{
	...
	#endif // RIODRAW_PATCH
	{
		...
		die("ddUI: execvp '%s' failed:", ((char **)arg->v)[0]);
	}
	#if RIODRAW_PATCH
	return pid;
	#endif // RIODRAW_PATCH
...
xerror(Display *dpy, XErrorEvent *ee)
{
	...
	|| (ee->request_code == X_CopyArea && ee->error_code == BadDrawable))
		return 0;
	fprintf(stderr, "ddUI: fatal error: request code=%d, error code=%d\n",
...
{
	die("ddUI: another window manager is already running");
	return -1;
}
...
main(int argc, char *argv[])
{
	...
	if (!(dpy = XOpenDisplay(NULL)))
		die("ddUI: cannot open display");
	#if SWALLOW_PATCH
	if (!(xcon = XGetXCBConnection(dpy)))
		die("ddUI: cannot get xcb connection\n");
	#endif // SWALLOW_PATCH
	checkotherwm();
